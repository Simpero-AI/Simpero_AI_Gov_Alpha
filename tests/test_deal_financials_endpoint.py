"""GET /deals/{deal_id}/financials -- the claims-driven Financials tab endpoint.

Locks the wire contract the unit tests (test_financials_view.py) don't cover: the
404 vs never-404-empty behavior, the camelCase response keys (including sourceUrl
on each fact), and the RLS tenancy that rests on the get_db dependency. Same
TestClient harness as tests/test_deal_market_endpoint.py.
"""
...
def seeded_org(owner_conn) -> Iterator[dict[str, Any]]:
    clerk_org_id = f"test-fin-org-{uuid.uuid4().hex[:8]}"
    with owner_conn.cursor() as cur:
        cur.execute(
            "INSERT INTO organisation (clerk_org_id, name, created_at) VALUES (%s, %s, now()) "
            "RETURNING id",
            (clerk_org_id, "Financials Endpoint Org"),
        )
...
def _seed_revenue_claim(owner_conn, org_pk: int, deal_id: str) -> None:
    value = {"raw": "$497.2M", "normalized": 497_200_000, "unit": "USD", "value_type": "currency"}
    with owner_conn.cursor() as cur:
        cur.execute(
            ...
            "VALUES (%s, %s, 'Acme', 'revenue', 'Revenue', %s::jsonb, 'pdf', 3, 100, 120, 'cited', "
            "'exact_span', 2023, 'A')",
...
def test_404_when_deal_missing(client, seeded_org):
    ...
    resp = client.get(f"/deals/{uuid.uuid4()}/financials")
    assert resp.status_code == 404


def test_empty_lists_when_no_financial_claims(client, seeded_org, seeded_deal):
    """A claim-less deal is a 200 with five empty section lists, never a 404 -- each
    section renders its own 'information not available' state."""
    _authed(seeded_org["clerk_org_id"], "user-1")
...
def test_returns_income_statement_with_camelcase_wire_keys(
    ...
    _seed_revenue_claim(owner_conn, seeded_org["org_pk"], seeded_deal)
    _authed(seeded_org["clerk_org_id"], "user-1")
    ...
    assert resp.status_code == 200
    body = resp.json()
    # camelCase section keys (the CamelModel contract the frontend consumes)
...
def test_scoped_to_the_caller_org(client, owner_conn, seeded_org, seeded_deal):
    ...
    other_clerk_org = f"test-fin-org-{uuid.uuid4().hex[:8]}"
    with owner_conn.cursor() as cur:
        cur.execute(
