...
| P3-15 | Vansh (picked up from Suraj, never started) | Done | `feat/p3-15-presign-content-length` | `staging` | **Yes — [PR #167](https://github.com/Simpero-AI/Simpero_AI_Gov_Alpha/pull/167)**, merged (`af5c478`) | Real Postgres (dev), fresh volume | P3-15/F9: `presign_put()` gains an optional `content_length` kwarg that binds the declared upload size into the SigV4 signature as an exact match, closing the last unenforced limit on the no-auth intake surface (previously only the client-declared `body.size` was checked, not the presigned URL itself). New `tests/test_presign_content_length.py` proves the signing property directly (real botocore SigV4 signer, no mocked signer): N and N+1 byte declarations produce different signatures. Two fix-prompt hardening rounds followed: (1) `/complete` now verifies the *actual stored* object size via new `head_object_size`, independent backstop in case Spaces doesn't honour the signed `Content-Length`; `PublicPresignRequest.size` gets `Field(ge=1)`. (2) oversized-stored-object rejection now writes a `human_audit_log` row (`intake_document_rejected`) before the 422, flagged in the PR for explicit sign-off as a new event type. 1141 passed / 2 skipped / 1 pre-existing unrelated failure (`test_missing_public_database_url_raises_validation_error`) + ruff clean + pyright 0 errors. **Manual verification against real Spaces — DONE (2026-08-30)**: verified using this repo's own `presign_put` (not a reimplementation) against the real bucket with a throwaway `_p3-15-verification/` key, since deleted. PUT body of exactly N (1024 bytes) → 200 (control); N+1 (1025 bytes) → 403 `SignatureDoesNotMatch`; N−1 (1023 bytes) → 403 `SignatureDoesNotMatch`. Confirms Spaces honours the signed `Content-Length` as an **exact match**, not a ceiling, same as AWS S3 — the failure is signature verification, not a separate size policy. The `/complete` stored-size check (item 1 above) stays as defence-in-depth regardless, since S3-compatibility details aren't contractual. This closes the PR's own "still needed" manual-verification ask. |
...
| X-03 | Vansh | Done | `x03-intake-injection-negative-coverage` | staging | **Yes — [PR #168](https://github.com/Simpero-AI/Simpero_AI_Gov_Alpha/pull/168)** | Real Postgres (dev, port 5434, `docker-compose.dev.yml`) + local Valkey (port 6381) | Closes the last gap in X-03 (Security pass: negative-path & injection coverage) — SQL-injection-shaped input handling on the public intake surface's free-text fields; everything else the ticket asks for already had coverage. New `tests/test_intake_injection_negative.py` (10 tests): 3 classic SQLi payloads round-tripped byte-for-byte inert through both `POST /answers` (draft save) and `POST /submit` (final answers), with `deal_intake_link`/`deal_intake_response` row counts and schema asserted unchanged directly via `owner_conn` — not just inferred from a 200. `IntakeEmailVerifyRequest.email`'s `EmailStr` "safe by construction" claim gets one explicit 422 assertion (using a real, valid link token, since `Depends(get_public_link_db)` resolves before FastAPI validates the request body — a garbage token would 404 before the email field is ever checked). **Real gap found and fixed, not just tested**: a literal null byte in `answer` wasn't actually "handled safely" — Postgres' jsonb input rejects an embedded null byte outright (`UntranslatableCharacterError`), so it 500'd at the DB layer instead of the ticket's expected 200. Fixed with a small schema-layer guard, `AnswerInput._reject_null_bytes` (`app/schemas/public_intake.py`), rejecting embedded null bytes with a clean 422 — same "safe by construction" posture already used for `email`. Flagged to Vansh before fixing (not assumed); approved. 10/10 new tests + full suite 1206/1208 (2 pre-existing, unrelated environmental failures — `test_dd_app_can_insert_and_select_human_audit_log`, dirty shared local dev volume; `test_missing_public_database_url_raises_validation_error`, the same pre-existing case P3-15/PR#167 already documented) + pyright 0 errors. Local test runs against `docker-compose.dev.yml`'s Postgres/Valkey needed `DATABASE_URL`/`PUBLIC_DATABASE_URL`/`ALEMBIC_DATABASE_URL`/`VALKEY_URL` overridden to `localhost:5434`/`localhost:6381` — `.env`'s own values (`pgbouncer:5432`, the DO-managed Valkey cluster) aren't reachable directly from a host `uv run pytest` invocation. |
